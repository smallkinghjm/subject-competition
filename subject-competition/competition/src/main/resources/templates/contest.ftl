    ...
    <script>
        layui.use(['jquery', 'form', 'layer', 'laydate', 'table'], function(){
            ...
            table.render({
                ...
                ,cols: [[
                    {field:'contestId', title:'编号',fixed: 'left',sort: true,id:'eventId'}
                    ,{field:'contestName', title:'标题', fixed: 'left',sort:true,templet: '<div><a href="/contest/view/{{d.contestId}}" class="layui-table-link">{{d.contestName}}</a></div>'}
                    ,{field:'contestStart', title:'开始时间', fixed: 'left',sort: true,
                        templet: function(res){
                            return layui.util.toDateString(res.contestStart, "yyyy-MM-dd");}}
                    ,{field:'state', title:'状态',  fixed: 'left',sort: true}
                ]]
            });
        });
    </script>
